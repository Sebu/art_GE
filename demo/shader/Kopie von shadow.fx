...
VS_OUTPUT VS(VS_INPUT V)
{
    ...
    float3 P = mul(float4(V.Pos, 1), (float4x3)WorldView);  // position (view space)
    Out.Pos  = mul(float4(P, 1), Projection);             // position (projected)


    float3 Z = mul(float4(V.Pos, 1), (float4x3)ShadowWVP);  // position (view space)
    float4 L = mul(float4(V.Pos, 1), (float4x4)LightWVP);  // position (view space)

    float3 N = normalize(mul(V.Norm, (float3x3)WorldView)); // normal (world space)
...
{
    float4 X =Spec-tex2Dproj(Sampler1,Tex).r;
